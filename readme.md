...
|[Edit MFA phone number](policies/restore-mfa-phone-number) | Demonstrates how to allow user to provide and validate a new MFA phone number. After the user changes their MFA phone number, on the next login, the user needs to provide the new phone number instead of the old one.|[Go](https://b2ciefsetupapp.azurewebsites.net/Home/Experimental?sampleFolderName=restore-mfa-phone-number)| [Live demo](policies/restore-mfa-phone-number#live-demo)|
|[Restore MFA phone number](policies/edit-mfa-phone-number) | Demonstrates how to allow user to change the phone in case it got lost. After the user changes their MFA phone number, on the next login, the user needs to provide the new phone number instead of the old one.|[Go](https://b2ciefsetupapp.azurewebsites.net/Home/Experimental?sampleFolderName=edit-mfa-phone-number)| [Live demo](policies/edit-mfa-phone-number#live-demo)|
